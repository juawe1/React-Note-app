...
export function NoteList({ availableTags, notes, updateTag, deleteTag }: NoteListProps) {
  ...
  return (
    <>
      ...
      <EditTagsModal show={editTagsModalIsOpen} handleClose={() => setEditTagsModalIsOpen(false)} availableTags={availableTags} updateTag={updateTag} deleteTag={deleteTag}/>
    </>
  );
}
...
function EditTagsModal({ show, handleClose, availableTags, updateTag, deleteTag }: EditTagsModal){
            ...
            <Form>
                <Stack gap={2}>
                    {availableTags.map(tag => (
                        <Row key={tag.id}>
                            <Col>
                                <Form.Control type="text" defaultValue={tag.label} onChange={e => {
                                  console.log(tag.id)
                                  updateTag(tag.id, e.target.value)
                                }}/>
                            </Col>
                            <Col xs="auto">
                                <Button variant="outline-danger" type="button" onClick={() => deleteTag(tag.id)}>&times;</Button>
                            </Col>
                        </Row>
                    ))}
                </Stack>
